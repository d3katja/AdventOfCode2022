...
# been assigned the job of cleaning up sections of the camp. Every section has a unique ID number, and each Elf
...
numberOfPairsThatFullyOverlap = 0
rowNumber = 0
...
for line in open("Day4/input4.txt"):
    ...
    print("numberOfPairsThatFullyOverlap: " + str(numberOfPairsThatFullyOverlap))
    print(elf1Assignment)
    print(elf2Assignment)

    # Lesson learned: string conversion gives a total of 508 pairs, must convert to inte before the comparison.
    if (
        int(elf1Assignment[0]) <= int(elf2Assignment[0])
        and int(elf1Assignment[1]) >= int(elf2Assignment[1])
        or int(elf2Assignment[0]) <= int(elf1Assignment[0])
        and int(elf2Assignment[1]) >= int(elf1Assignment[1])
    ):
        numberOfPairsThatFullyOverlap += 1
